/**
 * AstellrAuthProvider — global authentication context.
 *
 * Responsibilities:
 ...
 *   - astellr_platform_access_token
 *   - astellr_platform_refresh_token
 *   - astellr_platform_user (JSON)
 */
...
import type { AuthTokens, PlatformSession, PlatformUser } from '@/services/astellrPlatform';

// ─── Keychain keys ──────────────────────────────────────────────────────────

const KC_ACCESS_TOKEN = 'astellr_platform_access_token';
const KC_REFRESH_TOKEN = 'astellr_platform_refresh_token';
const KC_USER = 'astellr_platform_user';
...
interface AstellrAuthContextValue {
...
const AstellrAuthContext = createContext<AstellrAuthContextValue | undefined>(undefined);

export const useAstellrAuth = (): AstellrAuthContextValue => {
  const ctx = useContext(AstellrAuthContext);
  if (!ctx) {
    throw new Error('useAstellrAuth must be used within a AstellrAuthProvider');
  }
  return ctx;
};
...
export const AstellrAuthProvider: React.FC<React.PropsWithChildren> = ({ children }) => {
  ...
  const persistSession = useCallback(async (session: PlatformSession) => {
    ...
    if (previousUserId && previousUserId !== newUserId) {
      ...
      console.log(`[AstellrAuth] User switch detected: ${previousUserId} → ${newUserId}`);
   ...
   * The modal intercepts the `astellrwallet://oauth/callback` redirect,
  ...
  const loginWithOAuth = useCallback(
    async (provider: 'google' | 'apple') => {
      ...
      try {
        ...
        const result = await WebBrowser.openAuthSessionAsync(url, 'astellrwallet://oauth/callback');
  ...
  const logout = useCallback(async () => {
    ...
    if (currentTokens) {
      try {
        await fetch(`${ASTELLR_PLATFORM_URL}/api/auth/logout`, {
          method: 'POST',
          headers: {
  ...
  const value = useMemo<AstellrAuthContextValue>(
    () => ({
      isReady,
  ...
  return (
    <AstellrAuthContext.Provider value={value}>
      {children}
    </AstellrAuthContext.Provider>
  );
};
